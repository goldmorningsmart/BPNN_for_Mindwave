...
class BPNN(object):
    ...
    def __init__(self,input_n,hidden_n,output_n,lambd):
        ...
        self.hidden_weights =  tf.Variable(tf.random_normal((self.hidden_n,self.hidden_2),mean=0,stddev=1,dtype=tf.float64),trainable=True)                                          
        self.hidden_weights2 =  tf.Variable(tf.random_normal((self.hidden_2,self.hidden_3),mean=0,stddev=1,dtype=tf.float64),trainable=True) 
        self.hidden_weights3 =  tf.Variable(tf.random_normal((self.hidden_3,self.output_n),mean=0,stddev=1,dtype=tf.float64),trainable=True)
        ...
        self.hidden_cells2 = tf.sigmoid(tf.matmul(self.hidden_cells,self.hidden_weights)+self.hidden_threshold2)
        self.hidden_cells3 = tf.sigmoid(tf.matmul(self.hidden_cells2,self.hidden_weights2)+self.hidden_threshold3)
        self.output_cells = tf.sigmoid(tf.matmul(self.hidden_cells3,self.hidden_weights3)+self.output_threshold)
        # 定义损失函数,并加入损失集合
